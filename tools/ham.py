...
class VideoCamera02:
    ...
    def get_frame02(self, img):
        ...
        if hands:
            ...
            length, info, img = self.detector1.findDistance((lmList[4][0], lmList[4][1]), (lmList[8][0], lmList[8][1]), img)
            if length < 70:
                cursor = lmList[8]
                for idx, imgObject in enumerate(self.listImg):
                    if imgObject not in self.done_list:
                        ...
                        if 360 < imgObject.posOrigin[0] < 800 and 400 < imgObject.posOrigin[1] < 500 and imgObject.name not in self.orderList:
                            self.doneItem[imgObject] = self.plate_y - self.q
                            self.orderList.append(imgObject.name)
